...
class PayrollTab(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        self.employee_cb = ttk.Combobox(top, state="readonly", width=24)
        self.employee_cb.grid(row=0, column=1, padx=4)

        ttk.Label(top, text="From").grid(row=0, column=2, padx=4)
        self.from_entry = DateEntry(top, width=10, date_pattern="dd-mm-yyyy") if DateEntry else ttk.Entry(top, width=10)
        self.from_entry.grid(row=0, column=3, padx=4)

        ttk.Label(top, text="To").grid(row=0, column=4, padx=4)
        self.to_entry = DateEntry(top, width=10, date_pattern="dd-mm-yyyy") if DateEntry else ttk.Entry(top, width=10)
        ...
        ttk.Radiobutton(opts, text="Fixed (8 €/h)", variable=self.rate_mode, value="fixed")\
            .grid(row=0, column=0, padx=6)
        ttk.Radiobutton(opts, text="Custom rate", variable=self.rate_mode, value="custom")\
            .grid(row=0, column=1, padx=6)

        ttk.Separator(opts, orient="vertical").grid(row=0, column=2, sticky="ns", padx=8)

        ttk.Checkbutton(opts, text="Utilities", variable=self.utilities_enabled,
                        command=self._update_summary).grid(row=0, column=3)
        ttk.Entry(opts, width=6, textvariable=self.utilities_value)\
            .grid(row=0, column=4, padx=4)

        ttk.Checkbutton(opts, text="Rental", variable=self.rent_enabled,
                        command=self._update_summary).grid(row=0, column=5)
        ttk.Entry(opts, width=6, textvariable=self.rent_value)\
        ...
        self.tree.column("date", anchor="w", stretch=True, minwidth=200)
        self.tree.column("day", anchor="center", stretch=True, minwidth=160)
        self.tree.column("hours", anchor="center", stretch=True, minwidth=120)
        ...
        # ---------- SUMMARY ----------
        self.summary = ttk.Label(self, text="No deductions applied", style="Summary.TLabel")
        self.summary.grid(row=3, column=0, sticky="w", padx=16, pady=2)

        # ---------- BOTTOM ----------
        bottom = Card(self)
        bottom.grid(row=4, column=0, sticky="ew", padx=12, pady=6)
    ...
    def _generate_period(self):
        ...
        start = self._get_date(self.from_entry)
        end = self._get_date(self.to_entry)
        ...
        while cur <= end:
            weekday = cur.weekday()
            hours = "10.0" if weekday < 6 else "0"
            self.tree.insert(
                "",
                "end",
                values=(
    ...
    def _get_date(self, w):
        if hasattr(w, "get_date"):
            return w.get_date()
        return datetime.strptime(w.get(), "%d-%m-%Y")
    ...
    def _edit_hours(self, event):
        ...
        spin.set(self.tree.set(item, "hours") or "0")
    ...
    def _update_summary(self):
        parts = []
        if self.utilities_enabled.get():
            parts.append(f"Utilities: {self.utilities_value.get()} €")
        if self.rent_enabled.get():
            parts.append(f"Rental: {self.rent_value.get()} €")
        self.summary.configure(text=" | ".join(parts) if parts else "No deductions applied")
    ...
    def _preview_pdf(self):
        try:
            ...
            messagebox.showerror("PDF", "PDF preview service not found")
            return

        preview_payroll_pdf(
            employee_name=self.employee_cb.get(),
